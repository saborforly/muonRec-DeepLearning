...
def rotate_fxl(the,phi,vr):
    ...
    return root_the,root_phi
'''
def rotate_zyz(the,phi,rot_z1,rot_y,rot_z2):
    vec=ROOT.TVector3(sin(the)*cos(phi),sin(the)*sin(phi),cos(the)) 
    rz1=ROOT.TRotation().RotateZ(rot_z1)
    ry=ROOT.TRotation().RotateY(rot_y)
    rz2=ROOT.TRotation().RotateZ(rot_z2)
    rot_rz1=rz1*vec
    rot_ry =ry*rot_rz1
    rot=rz2*rot_ry
    root_the=rot.Theta()
...
def read(file,start):
    ...
    for serialized_example in tf.python_io.tf_record_iterator(file):
      ...
      while(num<1):
        ...
        #chage genstart
        gen_thes,gen_phis=rotate_zyz(genlabel[0], genlabel[1],rot_z1,rot_y,rot_z2)
        #change gendirection
        gen_thep,gen_phip=rotate_zyz(genlabel[2], genlabel[3],rot_z1,rot_y,rot_z2) 
        #chage recstart
        rec_thes,rec_phis=rotate_zyz(reclabel[0], reclabel[1],rot_z1,rot_y,rot_z2)
        #change recdirection
        rec_thep,rec_phip=rotate_zyz(reclabel[2], reclabel[3],rot_z1,rot_y,rot_z2)
        ...
        for the in range(shape[0]):
            for phi in range(shape[1]):
                if image[the][phi][1]>-0.5:
                    ...
                    the_new,phi_new=rotate_zyz(the_rad, phi_rad,rot_z1,rot_y,rot_z2)
