TString picsroot = "/junofs/users/liuyan2016/muon-sim/sim-nn/data-draw/";
...
void dD_mean_sigma(){
  ...
  double dD[4][nbins],dD_tt[4][nbins];
  double dD0[4][nbins],dD_cd[4][nbins];
  ...
  for(int i=0; i<nbins; i++){
    ...
    */
    //angle deg
    //valname = TString::Format("(geniy-reciy)>>hdD(100,-150,150)",usewhichrec);
    valname = TString::Format("(genDist-recDist)/10>>hdD(100,-150,150)",usewhichrec);
    TString valtitle = TString::Format("#alpha [deg] (%.2fm ~ %.2fm)",binb/1000,bine/1000);
    draw_tt(valname,valtitle,"hdD",selDesc,dD_tt[0][i],dD_tt[1][i],dD_tt[2][i],dD_tt[3][i]);

    draw_cd(valname,valtitle,"hdD",selDesc,dD_cd[0][i],dD_cd[1][i],dD_cd[2][i],dD_cd[3][i]);
    //
  }
  
  TCanvas* c_dxdydz =  new TCanvas;
  TGraphErrors* g_dx = new TGraphErrors(nbins,binc,dD_tt[0],errbin,dD_tt[1]);
  TGraphErrors* g_dy = new TGraphErrors(nbins,binc,dD_cd[0],errbin,dD_cd[1]);
  ...
  TGraphErrors* g_dthe = new TGraphErrors(nbins,binc,dD_tt[2],errbin,dD_tt[3]);
  TGraphErrors* g_dphi = new TGraphErrors(nbins,binc,dD_cd[2],errbin,dD_cd[3]);
  ...
  h2->SetTitle("");
  h2->GetYaxis()->CenterTitle();
  //h2->GetYaxis()->SetTitle("Mean #Delta iy [mm]");
  h2->GetYaxis()->SetTitle("Mean #Delta D [cm]");
  h2->GetYaxis()->SetRangeUser(-29,50);
  ...
  h1->SetTitle("");
  //h1->GetYaxis()->SetTitle("Sigma #Delta iy [mm]");
  h1->GetYaxis()->SetTitle("Sigma #Delta D [cm]");
  ...
  g_dphi->GetYaxis()->SetTitle("Sigma #Delta D [deg]");
  ...
  g_dthe->GetYaxis()->SetTitle("Sigma #Delta D [deg]");
