...
import junonn_inference_vgg16_feed as junonn_inference_vgg
import junonn_loss
...
def train():
    with tf.Graph().as_default():
        ...
        x_images,y_labels = junonn_inputs.inputs(eval_data=False, train_data = FLAGS.train_data)
        images = tf.placeholder(tf.float32, shape=[None,180,360,2], name="input")
        ...
        for step in xrange(old_step,FLAGS.max_steps):
            ...
            xs, ys = sess.run([x_images,y_labels])
            _, loss_value = sess.run([train_op, lossed],feed_dict={images: xs, labels: ys})
            ...
            if step % 200 == 0 or (step + 1) == FLAGS.max_steps:                
                ...
                graph_path = os.path.join(FLAGS.train_dir, 'expert-graph.pb')
                output_graph_def = graph_util.convert_variables_to_constants(sess,graph_def, ['prediction'] )                
                with tf.gfile.GFile(graph_path, 'wb') as fid:
