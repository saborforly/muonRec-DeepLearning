...
def inference(images):
    ...
    resh1 = tf.reshape(pool5, [-1, 36864])
    fc6=fc_op(resh1, 1024,name='fc6')
    fc7=fc_op(fc6, 512,name='fc7')
    fc8=fc_op(fc7, 256,name='fc8')

    with tf.variable_scope('softmax_linear') as scope:
        kernel=tf.get_variable("w", shape=[256,4], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        biases=tf.get_variable("b", shape=[4], dtype=tf.float32, initializer=tf.constant_initializer(0.0))
