...
def read_junonn(filename_queue):
    ...
    features = tf.parse_single_example(value,
        ...
        'label_raw':tf.FixedLenFeature([result.labellength],tf.float32), 'tag_raw':tf.FixedLenFeature([result.labellength],tf.float32),
                                  })
    ...
    tag_true=features['tag_raw'][0]

    
    result.label=tf.stack([label_true])
    result.tag=tf.stack([tag_true])
    return result


def _generate_image_and_label_batch(image, label,tag, min_queue_examples,
                                    batch_size, shuffle):
    # Create a queue that shuffles the examples, and then
    # read 'batch_size' images + labels from the example queue.
    num_preprocess_threads = 16
    if shuffle:
        images, label_batch, tag_batch = tf.train.shuffle_batch(
        [image, label, tag],
        ...
        images, label_batch, tag_batch = tf.train.batch(
        [image, label, tag],
        batch_size=batch_size,
        num_threads=num_preprocess_threads,
        capacity=min_queue_examples + 3 * batch_size)
    ...
    return images, tf.reshape(label_batch, [batch_size,label.get_shape()[0].value]),tf.reshape(tag_batch, [batch_size,tag.get_shape()[0].value])
...
def inputs_base(eval_data, train_data, batch_size):
    ...
    reshaped_tag = tf.cast(read_input.tag, tf.int32)

    height = read_input.height
    width = read_input.width

    # Generate a batch of images and labels by building up a queue of examples.
    return _generate_image_and_label_batch(reshaped_image, reshaped_label,reshaped_tag,
