...
void ana_txt_root()
{
  TString outdir ="/junofs/users/liuyan2016/muon-sim/sim-nn/data-ana/";
  ifstream fin;
  //cout<<fname<<endl;
  
  fin.open("/junofs/users/liuyan2016/muon-sim/sim-nn/data-ana/sim-cd-300k-vgg16-smear.txt");
  string str;

  TString outname =outdir+"anasim-cd-300k-vgg16-smear.root";
  ...
  double recix,reciy,reciz,recit;
  ...
  while (!fin.eof()){
    ...
    TVector3 rs(recix, reciy, reciz);
    TVector3 rdir(t_recpx, t_recpy, t_recpz);
    rdir = rdir.Unit();
    t_angle =(gdir.Angle(rdir))/TMath::Pi()*180; 
    ...
    TVector3 rscentrk = rs + (-rs*rdir)*rdir;
    double dist_cc = (rs.Cross(rdir)).Mag();// dist from track center to LS center
    TVector3 gr0 = rscentrk - TMath::Sqrt(m_LSRadius*m_LSRadius - dist_cc*dist_cc)*rdir;
    //t_recix = gr0[0];
    //t_reciy = gr0[1];
    //t_reciz = gr0[2];
    ...
    TVector3 rcentrk = rs + (-rs*rdir)*rdir;
    t_dist =  (gcentrk-rcentrk).Mag();

    t_genDist = gcentrk.Mag();
    t_recDist = rcentrk.Mag();
